...
class LiveStreamConsumer(AsyncWebsocketConsumer):
    ...
    async def connect(self):
        ...
        # Check if stream exists before accepting connection
    ...
    def check_stream_exists(self):
        ...
        try:
            LiveStream.objects.get(id=self.stream_id)
            return True
    ...
    def add_viewer(self):
        ...
        try:
            ...
            viewer, created = StreamViewer.objects.get_or_create(
                stream=stream,
                user=self.user,
                defaults={"is_currently_watching": True, "joined_at": timezone.now()},
            )
            ...
            stream.current_viewers = StreamViewer.objects.filter(
                stream=stream, is_currently_watching=True
            ).count()
        ...
        try:
            ...
            try:
                viewer = StreamViewer.objects.get(
                    stream=stream, user=self.user, is_currently_watching=True
                )
    ...
    def get_viewer_count(self):
        ...
        return StreamViewer.objects.filter(
            stream_id=self.stream_id, is_currently_watching=True
        ).count()
    ...
    def save_chat_message(self, message):
        ...
        try:
            ...
            chat_msg = StreamChat.objects.create(
                stream=stream, user=self.user, message=message
            )
    ...
    def save_qa_question(self, question_text):
        ...
        try:
            ...
            question = QAQuestion.objects.create(
                stream=stream, user=self.user, question=question_text
            )
